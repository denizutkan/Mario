...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ['walking'] = function(dt)
            ...
            if self.map:collides(self.map:tileAt(self.x - 1, self.y)) == 2 or
            self.map:collides(self.map:tileAt(self.x - 1, self.y + self.height - 1)) == 2 then
            ...
            elseif self.map:collides(self.map:tileAt(self.x + self.width, self.y)) == 2 or
                self.map:collides(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) == 2 then   
        ...
        ['jumping'] = function(dt)
            ...
            -- check if there's a tile directly beneath us
            if self.map:collides(self.map:tileAt(self.x, self.y + self.height)) == 1 or
            ...
            if self.map:collides(self.map:tileAt(self.x, self.y + self.height)) == 2 or
                self.map:collides(self.map:tileAt(self.x + self.width - 1, self.y + self.height)) == 2 then
...
function Player:update(dt)
    ...
    if self.y > map.mapHeight / 2 + 400 then

        self.y = map.mapHeight / 2 + 174
        self.x = self.x - 40
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= (TILE_EMPTY or CLOUD_RIGHT or CLOUD_LEFT) or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= (TILE_EMPTY or CLOUD_RIGHT or CLOUD_LEFT) then
            -- reset y velocity
            self.dy = 0
...
function Player:checkLeftCollision()
    if self.dx < 0 then
        ...
        if self.map:collides(self.map:tileAt(self.x - 1, self.y)) == 1 or
            self.map:collides(self.map:tileAt(self.x - 1, self.y + self.height - 1)) == 1 then
...
function Player:checkRightCollision()
    if self.dx > 0 then
        ...
        if self.map:collides(self.map:tileAt(self.x + self.width, self.y)) == 1 or
            self.map:collides(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) == 1 then
